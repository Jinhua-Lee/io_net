...
int main() {
    ...
    while (1) {
        ...
        int n_ready = select(maxfd + 1, &read_set, nullptr, nullptr, nullptr);
        if (n_ready < 0)
            perror("select error");
        ...
        if (FD_ISSET(listen_fd, &read_set)) {
            ...
            for (cli_index = 0; cli_index < FD_SETSIZE; cli_index++) {
                if (client[cli_index] < 0) {
                    client[cli_index] = conn_fd;
                    break;
                }
            }
        ...
        all_set = read_and_handle(client, all_set, read_set, max_valid_index, n_ready);
    }
}
